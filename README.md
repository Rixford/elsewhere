...
3. Open **Start Elsewhere.vbs**. Local mode works offline after setup; optional cloud models require internet.
...
Each installation starts with an empty browser and the local model. Setup creates its own runtime and data directory under the current Windows user's local application data; no developer account, personal path, saved page or API key is included. Use your own OpenAI or Anthropic API key in Settings if you want cloud generation. A ChatGPT or Claude subscription is not an API key.

Fork or clone the source, run setup on your own computer, then choose your theme and review preferences in Settings. For a reusable preference template, see [examples/settings.example.json](examples/settings.example.json) and its [instructions](examples/README.md). No source edits are needed to choose a model or use a different Windows username. The browser remains Windows-only; other operating systems are not tested.

See [PRIVACY.md](PRIVACY.md) before sharing diagnostics or packaging a fork. The public `rev0` tag preserves the initial release; `main` contains the subsequent improvements.
...
`runtime-path.txt` is generated by setup and points to that installation's model, runtime, isolated Python environment, and saved data. Windows app filesystem redirection can change the physical location. The launcher uses the detected path; this machine-specific file is excluded from Git and must not be copied into a release.
